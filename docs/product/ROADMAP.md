...
Ship order and detail: `ROADMAP_MINOR.md`.

- **v1.0.1:** Portion units g/ml + default custom amount 100 g (ml when unit is ml).
- **v1.0.2:** Email confirmation for production + Macrio-branded auth emails + forgot password.
- **v1.0.3:** Settings: edit username, app version, DOB calendar picker, date-format setting.
- **v1.0.4:** Onboarding/goals: calculator behind a button, “Niets/Nothing” allergens, soft body validation, macro autocomplete + kcal equivalents.
- **v1.0.5:** Diary: per-meal macro totals + progress toward daily goals.
- **v1.0.6:** Named portions (e.g. small / medium / large).
- **v1.0.7:** Cook state on products/versions: `cooked` | `uncooked` | `not_applicable` (rice, meat, pasta, etc.).
- **v1.0.8:** Locked parent foods + Dutch ~80% staple parent seed (pre-launch). Brief: `project-context/parent-food-catalog-pitch.md`.
